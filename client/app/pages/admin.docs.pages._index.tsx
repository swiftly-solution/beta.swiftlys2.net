...
import AdminDocumentationViewer from "~/elements/admin/docs/pages/AdminDocumentationViewer";
import PageWithNavbarLayout from "~/layouts/PageWithNavbar";
import { useUserStore } from "~/stores/user";

export function meta({ }: MetaArgs) {
    return [
        { title: "Admin Docs - Source 2 Server Framework" },
...
export default function Page() {
    ...
    return (
        <PageWithNavbarLayout head={<></>} navbar={<AdminNavigation />}>
            {
                !userStore.contentLoaded ? <LoaderIcon size={64} className="animate-spin" /> : <AdminDocumentationViewer />
            }
