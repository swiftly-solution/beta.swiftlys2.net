...
func FetchPage(c *types.Client, data map[string]any) types.Response {
	...
	if validationError != nil {
		return types.Response{Broadcast: false, Data: map[string]any{"event": "fetch-docs", "data": map[string]any{"status": 403, "message": validator.ProcessErrors(validationError)}}}
	}
	...
	if len(documentations) <= 0 {
		return types.Response{Broadcast: false, Data: map[string]any{"event": "fetch-docs", "data": map[string]any{"status": 404, "data": nil}}}
	} else {
		return types.Response{Broadcast: false, Data: map[string]any{"event": "fetch-docs", "data": map[string]any{"status": 200, "data": documentations[0]}}}
	}
}
